...
def read_img(path):
    ...
    for index in range(219):
        ...
        for im in glob.glob(path+str(index)+"/" + '/*.jpg'):

            #print('reading the images:%s' % (im))

            img = cv2.imread(im)             #調用opencv庫讀取像素點
            img = cv2.resize(img, (128, 128))  #圖像像素大小一致
...
xs = tf.placeholder(tf.float32, [None, 128, 128, 3])  #每張圖片32*32*3個點
    ...
    if train:
        ...
        with open('output.csv', 'w', newline='') as csvfile:
            ...
            # 真實label與模型預測label
                ...
                print(str(im)[-14:], " ", predicted_label_name)
